...
echo "CMA_NAME,Total_Memory_M,Total_Objects,Custom_Objects,NumPolices,TotalRulesPerCMA,NumNATRulesPerCMA" > CMA_DETAIL_$TIMESTAMP.csv
...
SID=$(mgmt_cli -r true login read-only true domain "$cma" --format json |jq -r '.sid')
...
MEMORY=$(ps aux |grep $cma | awk 'BEGIN { sum=0 } {sum=sum+$6; } END {printf("%s",sum / 1024)}')
echo "$cma,${MEMORY},${TOTAL},${CUSTOM},${PACKAGESNUM},${TOTALLAYERRULES},${NATRULETOTAL}" >> CMA_DETAIL_$TIMESTAMP.csv
# Do the right thing and logout after we are done and clear the session :)
echo ""
echo "Logging out of Read-Only session to R80 CMA $cma"
